...
@click.pass_context
def validate(ctx, check_platform, check_drp):
    """Validate API key and/or DRP token against the Rapid7 platform.

    \b
    Examples:
      r7-cli platform validate                # validate all configured credentials
      r7-cli platform validate --platform     # only check the Platform API key
      r7-cli platform validate --drp          # only check the DRP token
    """
